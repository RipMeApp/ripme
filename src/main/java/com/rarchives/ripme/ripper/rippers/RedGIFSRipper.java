...
public class RedGIFSRipper extends AbstractHTMLRipper {
    ...
    public boolean isProfile() {
        Pattern p = Pattern.compile("^https?://[wm.]*redgifs\\.com/watch/@([a-zA-Z0-9]+).*$");
    ...
    public Document getFirstPage() throws IOException {
        if (!isProfile()) {
            ...
            username = getGID(url);
            return Http.url(new URL("https://api.redgifs.com/v1/users/" +  username + "/gfycats")).ignoreContentType().get();
        }
    }
    ...
    public String getGID(URL url) throws MalformedURLException {
        Pattern p = Pattern.compile("^https?://(thumbs\\.|[wm\\.]*)redgifs\\.com/@?([a-zA-Z0-9]+).*$");
        Matcher m = p.matcher(url.toExternalForm());
