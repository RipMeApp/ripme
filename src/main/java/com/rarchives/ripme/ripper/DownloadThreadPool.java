...
public class DownloadThreadPool {
     ...
     * Tries to shutdown threadpool.
     *
     * expectedScheduledThreads/shouldStop are unused: crawler threads are always
     * submitted synchronously in the same thread, before this is called, so
     * there is nothing left to wait for them to be scheduled. Polling for
     * getScheduledThreadCount() to reach expectedScheduledThreads hung forever
     * for any ripper whose downloadURL() doesn't submit a crawler-pool thread
     * per item (e.g. rippers that call addURLToDownload() directly), since the
     * count would then never reach the expected value.
     */
